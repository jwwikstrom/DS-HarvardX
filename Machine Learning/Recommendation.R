...
movielens %>% mutate(week=wday(round_date(date, unit = "day"))) %>% group_by(week) %>% summarise(mean_rating = mean(rating)) %>% ggplot(aes(x=week, y = mean_rating)) + geom_line()

movielens %>% group_by(genres) %>% summarise(n_ratings = n(), mean_rating = mean(rating), sd_rating = sd(rating)) %>% filter(n_ratings>1000) %>% ggplot(aes(x=genres,y=mean_rating)) + geom_boxplot()
